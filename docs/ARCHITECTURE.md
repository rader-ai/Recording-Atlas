...
Import jobs receive a stable identity based on source bytes and processing settings. One worker runs at a time. Media duration is checked with ffprobe, then whisper-1 produces timed segments. Validated raw documents are saved before indexing. Whitespace normalization creates a separate readable version.

The existing retrieval core hashes passages and reuses unchanged vectors. A successful complete index is atomically replaced. Documents and the index are separate files, so this is not a transactional database. Resume reconstructs the document collection from saved transcripts. Operate only one process per data directory.
